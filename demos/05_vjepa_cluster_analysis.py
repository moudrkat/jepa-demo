"""
Demo 05 — V-JEPA 2 Temporal Cluster Analysis
==============================================

Downloads several short SSv2 clips (each showing a distinct hand-object action),
concatenates them into one video, then slides a temporal window across it to
extract V-JEPA 2 embeddings.  The embeddings are clustered (k-means) and
visualised with t-SNE so you can see how different actions form distinct regions
in representation space.

This is the "reference map" step — a later demo will project real-time video
...
def extract_embedding(model, processor, frames_16):
    ...
    Uses the last hidden state of the ViT backbone (before classification head),
    mean-pooled over all patch tokens.
    """
    inputs = processor(frames_16, return_tensors="pt").to(DEVICE)
    with torch.no_grad():
        outputs = model(**inputs)
    # Last hidden state: (1, n_patches, hidden_dim)
...
def plot_tsne_clusters(embeddings, cluster_labels, gt_labels, save_path):
    ...
    plt.suptitle("V-JEPA 2 Temporal Embedding Clusters", fontsize=17, fontweight="bold")
...
def plot_timeline(segment_labels, cluster_labels, window_centers, total_frames, save_path):
    ...
    gt_numeric = np.array([unique_actions.index(l) for l in segment_labels])
    ...
    plt.suptitle("Action Timeline: Ground Truth vs Discovered Clusters",
...
def plot_cluster_samples(video, cluster_labels, window_centers, save_path):
    ...
    plt.suptitle("Representative Frames per Cluster", fontsize=15, fontweight="bold")
...
def main():
    print("=" * 55)
    print("  Demo 05 — V-JEPA 2 Temporal Cluster Analysis")
    print("=" * 55)
    ...
    print(f"  Saved embeddings → {OUTPUT_DIR / '05_embeddings.npz'}")

    # 4. Visualise
    print("\n[4/4] Creating visualisations...")
    plot_tsne_clusters(
        embeddings, cluster_labels, gt_labels,
        OUTPUT_DIR / "05_tsne_clusters.png",
    )
    plot_timeline(
        segment_labels, cluster_labels, window_centers, len(video),
        OUTPUT_DIR / "05_timeline.png",
    )
    plot_cluster_samples(
        video, cluster_labels, window_centers,
        OUTPUT_DIR / "05_cluster_samples.png",
    )

    print(f"\n  Done! Check {OUTPUT_DIR}/05_*.png")
    print("  Embeddings saved to 05_embeddings.npz for real-time matching later.")
